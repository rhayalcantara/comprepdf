...
def _stamp_drawn_signature(
    ...
    with pikepdf.open(input_pdf) as pdf:
        ...
        for idx in indices:
            page = pdf.pages[idx]
            rect_w, rect_h, px, py = _page_rect(page, x, y, w, aspect)
            ...
            ops = f"q {rect_w:.4f} 0 0 {rect_h:.4f} {px:.4f} {py:.4f} cm {name} Do Q\n"
...
def _page_rect(page: pikepdf.Page, x: float, y: float, w: float, aspect: float) -> Tuple[float, float, float, float]:
    """Rect (w, h, x, y) en puntos para ESTA página, con clamp dentro de ella."""
    ...
    if rect_h > ph:  # no cabe a lo alto: reducir manteniendo el aspect ratio
        rect_h = ph
    ...
    py = max(lly, min(py, ury - rect_h))
    return rect_w, rect_h, px, py
